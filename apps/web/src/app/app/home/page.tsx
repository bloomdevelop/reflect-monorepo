...
import type { User, Channel } from "revolt.js";
import { Avatar, AvatarImage, AvatarFallback } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { MessageCircle, UserCircle2, Settings, Mail } from "lucide-react";
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                ...
                const directMessages = channels.filter(channel => 
                    channel.type === "DirectMessage" && channel.active
                );
                setDms(directMessages);

                // Get friends
                const friendsList = Array.from(client.users.values()).filter(user => 
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-muted/40 rounded-lg divide-y divide-muted">
                        {dms.length === 0 ? (
                            ...
                            dms.map(dm => {
                                const recipient = Array.from(dm.recipients || [])
                        ...
                        {friends.length === 0 ? (
                            ...
                            friends.map(friend => (
                                <div key={friend.id} className="flex items-center justify-between p-3">
                                    <div className="flex items-center gap-3">
                                        <Avatar>
                                            <AvatarImage src={friend.avatarURL || friend.defaultAvatarURL} />
                                            <AvatarFallback>
